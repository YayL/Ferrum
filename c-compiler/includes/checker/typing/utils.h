...
static char is_free_from_typevar(ID type_id) {
	switch (type_id.type) {
		case ID_TYPE_VAR: return 0;
		case ID_INVALID_TYPE:
		case ID_VOID_TYPE:
		case ID_NUMERIC_TYPE: return 1;
		case ID_EXISTENIAL: return is_free_from_typevar(LOOKUP(type_id, Existential).solved_type);
		case ID_REF_TYPE: return is_free_from_typevar(LOOKUP(type_id, Ref_T).basetype_id);
		case ID_ARRAY_TYPE: return is_free_from_typevar(LOOKUP(type_id, Array_T).basetype_id);
		case ID_PLACE_TYPE: return is_free_from_typevar(LOOKUP(type_id, Place_T).basetype_id);
		case ID_FN_TYPE: {
			Fn_T fn_type = LOOKUP(type_id, Fn_T);
			return is_free_from_typevar(fn_type.arg_type) && is_free_from_typevar(fn_type.ret_type);
		}
		case ID_TUPLE_TYPE: {
			Tuple_T tuple_type = LOOKUP(type_id, Tuple_T);
			for (size_t i = 0; i < tuple_type.types.size; ++i) {
				if (!is_free_from_typevar(ARENA_GET(tuple_type.types, i, ID))) {
					return 0;
				}
			}
		...
		case ID_SYMBOL_TYPE: {
			Symbol_T symbol_type = LOOKUP(type_id, Symbol_T);
			for (size_t i = 0; i < symbol_type.templates.size; ++i) {
				if (!is_free_from_typevar(ARENA_GET(symbol_type.templates, i, ID))) {
					return 0;
				}
			}
